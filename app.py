...
#     thread = threading.Thread(target=run_scraper, args=(platform, target, limit))
#     thread.daemon = True
#     thread.start()
#
...
#
#
# def run_scraper(platform, target, limit):
#     global scraping_status
#
#     try:
#         csv_filename = None
...
#     global scraping_status
#
#     if not scraping_status['csv_file'] or not os.path.exists(scraping_status['csv_file']):
#         return jsonify({'success': False, 'error': 'No data file found'})
#
#     try:
#         csv_data = []
#         with open(scraping_status['csv_file'], 'r', encoding='utf-8') as f:
#             reader = csv.reader(f)
...
#
#     filename = f"extracted_data_{scraping_status['platform']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
#
#     return send_file(
#         scraping_status['csv_file'],
#         as_attachment=True,
...
#         args=(sender_email, sender_password, recipient_email, smtp_server, smtp_port)
#     )
#     thread.daemon = True
#     thread.start()
#
...
#
#
# def send_email_smtp(sender_email, sender_password, recipient_email, smtp_server, smtp_port):
#     global scraping_status
#
#     try:
#         # Create message
...
#         msg['Subject'] = f"Scraped Data from {scraping_status['platform'].title()}"
#
#         # Email body
#         body = f"""Hello,
#
# Please find attached the scraped data from {scraping_status['platform'].title()}.
#
# Scraping Details:
# - Platform: {scraping_status['platform'].title()}
# - Target: {scraping_status['target']}
# - Limit: {scraping_status['limit']}
# - Records: {scraping_status['record_count']}
...
#
#         # Attach CSV file
#         if os.path.exists(scraping_status['csv_file']):
#             with open(scraping_status['csv_file'], "rb") as attachment:
...
#             part.add_header(
#                 'Content-Disposition',
#                 f'attachment; filename= {os.path.basename(scraping_status["csv_file"])}'
#             )
#             msg.attach(part)
#
#         # Send email
